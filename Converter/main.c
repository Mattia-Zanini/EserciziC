...
int main(int argc, char *argv[]) {
  ...
  if (GoodExtension(argv[1]) != 0) {
    ...
    DoLog("Error while translating, error code: 3; input file haven't the "
          "right extension");
    return 3;
  }
  ...
  int fileSize = atoi(nchar);
  char file[fileSize];

  // reading of the file pass by argument
  FILE *fp;
  char t;
  ...
  fclose(fp);
  // EXPERIMENTAL show the contenent of the array
